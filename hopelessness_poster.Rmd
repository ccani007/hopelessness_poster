...
age <- hopelessness_analysis_srv %>%
  group_by(Q25) |>
  summarise(mean = survey_mean(Age, na.rm = TRUE),
...
other_race <- race |>
  filter(Variable %in%
           c("Asian","Native Hawaiian/Other PI",
             "Am Indian/Alaska Native")) |>
  summarise(across(-Variable, sum)) |>
  mutate(Variable= "Other")

final_race <- race |>
  filter(!Variable %in% c("Asian",
...
bind_rows(sex, final_race, hopeless,  sexualOrientation) |>
  ...
  tab_row_group(
    label = md("**Race**"),
    rows = c("Black or African American", "Hispanic/Latino",
...
metric_table <-
  ...
  rename("Kappa" = "kap", "Accuracy" = "accuracy", "AUC" = "roc_auc",
...
bind_rows(lasso_vip, randomForest_vip, xgboost_vip) |>
  ...
  scale_y_reverse(label = c("Most Important", "2nd", "3rd", "4th", "5th", 
                            "6th", "7th", "8th", "9th", "10th"), breaks = c(1:10)) +
